...
(function(){
  ...
  function fit(sig){
    ...
    var framed=sig.parentElement&&sig.parentElement.matches(selector);
    ...
    photos.forEach(function(photo){
      var img=photo.querySelector('img');if(!img||(!framed&&(!img.naturalWidth||!img.naturalHeight)))return;
      var ratio=framed?(portrait?2/3:square?1:16/10):square?1:img.naturalWidth/img.naturalHeight;
      ...
      if(framed)set(photo,'height',(Math.floor(width/ratio*100)/100)+'px');
    });
    if(photos.length===1){
  ...
  function boot(){
    ...
    document.documentElement.setAttribute('data-mm-signatures','1.5.0');
  }
